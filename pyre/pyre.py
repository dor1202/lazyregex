...
class PyreApp(App[ReturnType]):
    CSS_PATH = "pyre.tcss"

    AVAILABLE_MODES = ["searh", "match", "replace"]
    ...
    def __init__(
        ...
        initial_mode = initial_mode or PyreApp.AVAILABLE_MODES[0]
        if initial_mode not in PyreApp.AVAILABLE_MODES:
            raise ValueError(
                f"This regex mode isn't supported!"
                f"please choose from the following list: {PyreApp.AVAILABLE_MODES}"
            )

        super().__init__()
        self.input_content: str = input_content
        self.regex_modes: list[str] = PyreApp.AVAILABLE_MODES
